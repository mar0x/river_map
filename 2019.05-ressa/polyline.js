...
function update_distance() {
    ...
    while (!i.is(end)) {
        ...
        for (var j in myStops) {
            var s = myStops[j];
            if (i.is(s.properties.get('nearest'))) {
                s.properties.set('dist', last_stop);
                s.properties.set('hintContent', Math.round(last_stop) + ' м');
                last_stop = 0;
            }
        }
    ...
    myEnd.properties.set('hintContent', Math.round(res) + ' м');
...
function init() {
    jQuery.get(xml_name, function(doc) {
        ...
        myStops = [];
        if (typeof(window["int_coords"]) == "object") {
            var a = window.int_coords;
            var n = a.length;
            for (var i = 0; i < n; i++) {
                var stop_at = find_nearest(a[i]);

                myStops.push(new ymaps.GeoObject({
                        geometry: {
                            type: "Point",
                            coordinates: stop_at.point,
                        },
                        properties: {
                            iconContent: 'Стоянка №' + (i + 1),
                            // hintContent: 'Ну давай уже тащи',
                            nearest: stop_at,
                        }
                    }, {
...
//                zoom: 10,
        ...
        for (var i in myStops) {
            myMap.geoObjects.add(myStops[i]);
        }
